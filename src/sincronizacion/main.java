...
public class main {
    ...
    public static int puerto, store_id, syncronization_interval, depuration_interval;//puerto de socket para no dejar abrir mas de una vez el sistema
    public static long retardo_inicial;//retardo antes de iniciar la sincroinizacion
    public static String local_system_path, store_name, depuration_time;//nombre de la carpeta de sistema General en local
    ...
    public static void main(String[] args) throws SQLException, IOException, FileNotFoundException, InterruptedException {
        ...
        if (!arch.exists()) {
System.out.println("Entra en archivo no existe");
            carga_inicial.crea_config();
        } else {
            carga_inicial.leer_ruta();
        }

        while(!carga_inicial.carga_inicial(ruta_config) ){//consulta archivo de configuracion
            carga_inicial.crea_config();
        }
        try{
        ...
        new ventanaInicio(ruta_config, retardo_inicial, local_system_path, store_id, syncronization_interval, store_name, depuration_time, depuration_interval);//int store_id, int syncronization_interval, String store_name
    }
